...
<template>
  <header class="sticky top-0 z-100 bg-(--color-bg)/92 backdrop-blur-md border-b border-(--color-border)">
    <div class="container-page flex items-center justify-between gap-6 py-4">
      ...
      <nav class="hidden min-[1150px]:flex items-center gap-8">
        <NuxtLink
      ...
      <div class="flex items-center gap-3.5">
        ...
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.2.2 2.4.6 3.6.1.4 0 .8-.2 1l-2.3 2.2z" fill="#fff" /></svg>
        ...
        >
          <svg v-if="!menuOpen" width="26" height="26" viewBox="0 0 24 24" fill="none"><path d="M3 6h18M3 12h18M3 18h18" stroke="#1C1917" stroke-width="2" stroke-linecap="round" /></svg>
          <svg v-else width="26" height="26" viewBox="0 0 24 24" fill="none"><path d="M6 6l12 12M18 6L6 18" stroke="#1C1917" stroke-width="2" stroke-linecap="round" /></svg>
        </button>
      </div>
    </div>

    <div v-if="menuOpen" class="min-[1150px]:hidden border-t border-(--color-border) px-5 sm:px-8 py-4 flex flex-col gap-4 bg-(--color-bg)">
      <NuxtLink
        v-for="link in navLinks"
        :key="link.to"
        :to="link.to"
